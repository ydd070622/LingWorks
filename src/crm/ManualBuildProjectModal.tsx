...
import type { ManualBuildProjectInput } from './types'
import { today } from './helpers'

interface Props {
  designers: string[]
  onSave: (input: ManualBuildProjectInput) => void
  onClose: () => void
}

export default function ManualBuildProjectModal({ designers, onSave, onClose }: Props) {
  const [form, setForm] = useState<ManualBuildProjectInput>({
    projectName: '',
    name: '',
    phone: '',
    community: '',
  ...
  const update = (key: keyof ManualBuildProjectInput, value: string) => {
    ...
    <div className="proj-modal-overlay">
      <div className="proj-modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <label>项目名称 *</label>
            <input className="proj-input" value={form.projectName} onChange={e => update('projectName', e.target.value)} placeholder="如：翡翠湾花园施工" />
          </div>
          <div>
            <label>业主 *</label>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
          <div>
            <label>本周施工内容</label>
            <textarea className="proj-input" rows={3} value={form.thisWeekWork} onChange={e => update('thisWeekWork', e.target.value)} />
          </div>
          <div>
            <label>本周主材进场</label>
            <textarea className="proj-input" rows={3} value={form.thisWeekMaterials} onChange={e => update('thisWeekMaterials', e.target.value)} />
          </div>
          <div>
            <label>本周遗留问题</label>
            <textarea className="proj-input" rows={3} value={form.thisWeekIssues} onChange={e => update('thisWeekIssues', e.target.value)} />
          </div>
          <div>
            <label>下周计划施工内容</label>
            <textarea className="proj-input" rows={3} value={form.nextWeekPlan} onChange={e => update('nextWeekPlan', e.target.value)} />
          </div>
        </div>
        <div style={{ marginTop: 12 }}>
          <label>备注</label>
          <textarea className="proj-input" rows={3} value={form.remark} onChange={e => update('remark', e.target.value)} placeholder="施工备注" />
        </div>
